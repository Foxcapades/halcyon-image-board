...
if($USR['level'] >= $BINFO['allowed']) {
	// Set some stuff for the template
	$frmvrs=array('action'=>$_SERVER['REQUEST_URI'].'&amp;post=new','unme'=>$USR['name']);

	// Load the template and add it to the $body var
	$body .= $P->formtovar('ninja','forms.php','newthread',$frmvrs);
}
